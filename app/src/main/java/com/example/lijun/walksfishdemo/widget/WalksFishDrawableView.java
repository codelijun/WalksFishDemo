...
public class WalksFishDrawableView {
    private static final boolean DEBUG = true;
    private static final String TAG = "WalksFishDrawable";
    ...
    private final int mFishHeadRadius;

    //鱼身体部位的坐标
    private PointF mBodyEndPoint;
    private final int mFishBodyLength;
    ...
    private final int mFishFinsLength;
    ...
    //鱼尾的长度
    private final int mFishTailLength;
    //鱼尾张开的最大宽度
    private final int mFishTailMaxWidth;
    private final int mFishTailInterval;

    public WalksFishDrawableView(Context context) {
        this.mContext = context;
        this.mPaint = new Paint();
        this.mPath = new Path();
        this.mPaint.setAntiAlias(true);
        this.mPaint.setColor(0xffff3300);
        ...
        mMiddlePoint = new PointF(getIntrinsicWidth() / 2f, getIntrinsicHeight() / 2f);
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        if (DEBUG) {
            Log.d(TAG, " draw() mDynamicAngle== " + mDynamicAngle);
        }
        drawFishHead(canvas);
        drawFishBody(canvas);
